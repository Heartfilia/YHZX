...
class Detail(object):
    ...
    def headers_get(self, resume_no):
        from spider import cookies
        cookie = importlib.reload(cookies).cookie
        headers = {
            'Accept': 'application/json, text/javascript, */*; q=0.01',
    ...
    def deal_info(self, info):
        ...
        personal_home_page = job["jobTitle"]
        excecutiveneed = ''
        ...
        update_date = time.strftime('%Y-%m-%d', time.localtime(int(jobResume['modifieddate']) / 1000))
    ...
    def post_resume(self, jr, account):
        info = {
            ...
            'job_name': jr.get('personal_home_page'),
            'data': [jr]
        }
        print(info)
    ...
    def params_pg_get(self, i, x):
        ...
        params = {
            ...
            "S_CreateDate": f"191029,{today_info}",
    ...
    def first_time(self, i):
        # 获取数据 返回总共有多少页
        params = self.params_pg_get(0, i)
    ...
    def stabilization(self, pg, i):
        # 对每一页的数据进行遍历
        for x in range(pg):
            LOG.info(f'当前所在页为第{x+1}页')
            params = self.params_pg_get(x, i)
            # time.sleep(11)
            try:
                time.sleep(random.uniform(1, 3))
    ...
    def run(self):
        for x in [2, 1]:
            try:
                pg = self.first_time(x)
                LOG.info(f'获取页面数据有{pg}页')
                self.stabilization(pg, x)
            except Exception:
                LOG.warning('cookies 失效！')

                msg = f"""
    ********* HR 数据自动化 *********\n负责人：{handler}\n状态原因：智联{company_name}主动投递简历获取程序发现异常\n处理标准：请人为到服务器查看是否有验证码或者是否需要替换本地cookie,有困难可以找相关技术人员协助处理
    """
    ...
    def work_time(func):
        ...
        def wrapper(*args, **kwargs):
            if set_time not in ['Sun']:
                ...
                if t in ['08:10', '12:26', '23:50', '16:15', '18:15']:
                    func(*args, **kwargs)
                else:
                    print(f'\r{t}', end='')
        return wrapper
...
def main():
    ...
    try:
        ...
        app.send_rtx_msg(f'********* HR 数据自动化 *********\n负责人：{handler}\n状态原因：智联{company_name}需要处理验证码')
