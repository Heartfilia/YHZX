...
| ------------------- | ------ | ------ | --------- | ---------- | ----- | -------- |
| `Job51_yhzx_shelly` | shelly | 张子珏 | `yhzx151` | `配置文件` | 23355 | 银河在线 |
| `Job51_wbdz_ling`   | ling   | 杨国玲 | `gzwb537` | 配置文件   | 25533 | 外宝电子 |
| `Job51_ssm_linda`   | linda  | 陈淼灵 | `ymrj483` | 配置文件   | 23333 | 时时美   |
...
- 主动下载
  ...
  - 功能：检测每天的主动下载的简历
  - 描述：因为采用了文字加密，所以程序是采用了自动化加`PIL` 和 `opencv`方法，经过优化，识别加密的一条信息会10秒(纯数字)，准确率99以上
- 注意问题
